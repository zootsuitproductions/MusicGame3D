...
namespace MidiPlayerTK
{
    ...
    public class MidiCommonEditor : ScriptableObject
    {
        ...
        static public bool LoadSkinAndStyle(bool loadSkin = true)
        {
            bool needStyleReloaded = false;
            if (loadSkin)
            {   
                if (skin == null || skin.name != "MaestroSkin")
                {
